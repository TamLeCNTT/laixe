...
const Paginations = (props) => {
      ...
      page.map((item, index) => {
        if (TongST > 8) {
          if (index >= (present - 2) && index <= (present + 2)) {
            console.log(index)
            list.push(<a key={index} href="#" value={item} onClick={() => changechoose(item)} className={choose == item ? "active" : ""}>{item + 1}</a>)
  ...
  return (
    ...
    <>
      ...
      {
        page.length > 0 && (<div id="pagination" className="pagination">
          ...
          {/* {show(TongST, choose)} */}
          {
            ...
            page.map((item, index) => {
              return (
                ((index >= (choose - 2) && index <= (choose + 2)) || (index <= TongST && index > TongST - 5)) ?
                  <a key={index} href="#" value={item} onClick={() => changechoose(item)} className={choose == item ? "active" : ""}>{item + 1}</a>
                  :
